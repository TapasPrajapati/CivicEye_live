...
class MyCasesManager {
    ...
    updateStats() {
        ...
        const pendingCases = this.cases.filter((case_) =>
            ["registered", "approved", "officer-assigned", "investigating"].includes(
                case_.status
            )
        ).length;
        const resolvedCases = this.cases.filter(
    ...
    renderCases() {
        ...
        casesList.innerHTML = this.filteredCases
            .map((case_) => this.createCaseCard(case_))
            .join("");
        ...
        return `
            <div class="case-card">
                <div class="case-header">
                    ...
                    <span class="case-status ${statusClass}">${this.getStatusLabel(
            case_.status
        )}</span>
                </div>
                ...
                <div class="case-actions">
                    <a href="/Frontend/Track_case/track.html?caseId=${case_.reportId
            }" class="btn-case-action btn-track-case">
    ...
    setupEventListeners() {
        ...
        // Crime type filter
        document
            .getElementById("crimeTypeFilter")
    ...
    applyFilters() {
        this.filteredCases = this.cases.filter((case_) => {
            const statusMatch =
                ...
                case_.status === this.currentFilters.status;
            const crimeTypeMatch =
                this.currentFilters.crimeType === "all" ||
                case_.crimeType === this.currentFilters.crimeType;
