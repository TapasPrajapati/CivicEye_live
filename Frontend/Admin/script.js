...
const mockFIRs = [
  ...
  { id: 1005, type: "Theft", location: "Shopping Mall", reporter: "David Lee", date: "2024-01-11", status: "Reported" },
  {
    id: 1006,
    type: "Assault",
...
const mockOfficers = [
  { badge: "B001", name: "Officer Smith", rank: "Sergeant", department: "Patrol", status: "On Duty" },
  { badge: "B002", name: "Officer Johnson", rank: "Constable", department: "Investigation", status: "Off Duty" },
  { badge: "B003", name: "Officer Wilson", rank: "Inspector", department: "Traffic", status: "On Duty" },
  { badge: "B004", name: "Officer Brown", rank: "Constable", department: "Patrol", status: "On Duty" },
  { badge: "B005", name: "Officer Lee", rank: "Sergeant", department: "Investigation", status: "Off Duty" },
  { badge: "B006", name: "Officer Davis", rank: "Constable", department: "Traffic", status: "On Duty" },
]
...
function populateFIRTable() {
  ...
  mockFIRs.forEach((fir) => {
    ...
    row.innerHTML = `
            ...
            <td>${fir.reporter}</td>
            <td>${fir.date}</td>
            <td><span class="status-badge ${fir.status.toLowerCase().replace(" ", "")}">${fir.status}</span></td>
            <td>
                <div class="action-buttons">
                    <button class="action-btn view" onclick="viewFIR(${fir.id})" title="View">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="action-btn edit" onclick="editFIR(${fir.id})" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="action-btn delete" onclick="deleteFIR(${fir.id})" title="Delete">
...
function populateUserTable() {
  ...
  paginatedUsers.forEach((user) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <div class="action-buttons">
                    <button class="action-btn view" onclick="viewUser('${user.email}')" title="View">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="action-btn edit" onclick="editUser('${user.email}')" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                    <div class="toggle-switch">
                        <input type="checkbox" id="user-${user.email}" ${user.status === "Active" ? "checked" : ""} 
                               onchange="toggleUserStatus('${user.email}')">
                        <label for="user-${user.email}" class="toggle-label"></label>
                    </div>
                </div>
            </td>
        `
...
function populateOfficerTable() {
  ...
  mockOfficers.forEach((officer) => {
    ...
    row.innerHTML = `
            ...
            <td><span class="status-badge ${officer.status.toLowerCase().replace(" ", "")}">${officer.status}</span></td>
            <td>
                <div class="action-buttons">
                    <button class="action-btn view" onclick="viewOfficer('${officer.badge}')" title="View">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="action-btn edit" onclick="editOfficer('${officer.badge}')" title="Edit">
                        <i class="fas fa-edit"></i>
                    </button>
                </div>
            </td>
...
function populateLogTable() {
  ...
  mockLogs.forEach((log) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <button class="btn-secondary" onclick="viewLogDetails('${log.timestamp}')">
...
function filterLogs() {
  ...
  const filteredLogs = mockLogs.filter((log) => {
    const matchesSearch = log.user.toLowerCase().includes(searchTerm) || log.action.toLowerCase().includes(searchTerm)
